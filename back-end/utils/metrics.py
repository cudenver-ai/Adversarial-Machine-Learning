...
def update_evalMetrics():
    ...
    metrics = [
        {"title": label, "data": [], "interval": "daily", "trend": ""}
    ...
    try:
        for entry in submissions:
            ...
            if parsed_timestamp in history.keys():
                history[parsed_timestamp]["incorrect_ratio"].append(
                    entry["incorrect_ratio"]
                )
                history[parsed_timestamp]["avg_confidence_incorrect"].append(
                    entry["avg_confidence_incorrect"]
                )
                history[parsed_timestamp]["avg_l2_perturbation"].append(
                    entry["avg_l2_perturbation"]
                )
                history[parsed_timestamp]["avg_ssim"].append(entry["avg_ssim"])
                history[parsed_timestamp]["avg_confidence_gap"].append(
                    entry["avg_confidence_gap"]
                )
    ...
    try:
        ...
        elapsed_days = [
            start_date + timedelta(days_elapsed)
            for days_elapsed in range((today - start_date).days + 1)
        ]
...
def update_leaderBoard():
    ...
    try:
        for entry in submissions:
            if entry["team_name"] in history:
                ...
                history[entry["team_name"]]["timestamp"].append(
                    datetime.strptime(entry["time_stamp"], "%Y-%m-%d %H:%M:%S")
                )
            else:
    ...
    try:
        for team in teams:
            sorted_team_metrics = sorted(
                zip(history[team]["score"], history[team]["timestamp"]),
    ...
    try:
        sorted_overall_metrics = sorted(
            zip(teams, top_scores, top_timestamps),
            key=lambda x: (x[1], -x[2].month, -x[2].day),
            reverse=True,
        )
    except Exception as e:
